...
const visibleCount = computed(() => {
  return Math.ceil(screenHeight.value / props.estimatedItemSize) // 容器高度/预估高度=一屏显示数量
})
...
// watch(visibleData, (newVal) => {
//   emit('update:visibleCount', newVal.length)
// }, { immediate: true })
...
const updateItemsSize = () => {
  ...
  nodes.forEach((node) => {
    ...
    // --- 新增：打印修正后的真实高度 ---
    console.log(`第 ${index} 项 真实高度: ${height}px, 预估高度: ${oldHeight}px, 差值: ${dValue}px`)
  })
}
...
const scrollEvent = () => {
  ...
  console.log('计算出的 start 索引:', getStartIndex(scrollTop))
  // 打印当前 start 项的 top/bottom 位置
  const startIndex = getStartIndex(scrollTop)
  if (positions.value[startIndex]) {
    console.log('start 项的 top:', positions.value[startIndex].top)
    console.log('start 项的 bottom:', positions.value[startIndex].bottom)
  }
  // --- 打印结束 ---
